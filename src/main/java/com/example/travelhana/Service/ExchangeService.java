...
public class ExchangeService {
    ...
    public ResponseEntity<?> exchangeInAccountBusinessDay(ExchangeRequestDto dto)
    {
        try{
            ...
            if(keyMoney==null) {
                keyMoney=  makeKeyMoney(account.getUser(), dto.getUnit());
            }
            ...
            if(dto.getIsBought()){ //원화->외화 요청 : 원화계좌 확인
                ...
                if(dto.getMoney()>keyMoney.getBalance()){
                    throw new BusinessExceptionHandler(ErrorCode.INSUFFICIENT_BALANCE);
                }
            }

            //환전 시작
            ExchangeResponseDto exchangeHistory = saveExchangeThings(keyMoney,account,dto); //키머니, 원화계좌, 처리할요청
    ...
    public ExchangeResponseDto saveExchangeThings(KeyMoney keymoney,Account account,ExchangeRequestDto dto)
    {
        ...
        if(dto.getIsBought()) { //원화 -> 외화
            ...
            exchangeResult=keyToWon(money,keymoney,account,rate); //money=외화
        }
    ...
    public ExchangeSuccess keyToWon(Long key, KeyMoney keyMoney, Account account, Double rate)
    {
        ...
        keyMoney.updateBalance(key*(-1)); //키머니 잔액 차감
        account.updateBalance(realwon); //원화 잔액 추가

        return new ExchangeSuccess(realwon,key,keyMoney.getBalance(),false,keyMoney.getBalance());
    }
